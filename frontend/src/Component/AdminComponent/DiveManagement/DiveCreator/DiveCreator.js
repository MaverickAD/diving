...
function DiveCreator(props) {
  ...
  const handleDeletePalanquee = (index) => {
    ...
    let indexElem = updatedPalanquees.findIndex(
      (palanquee) => palanquee.id === index
    );
    let skip = false;
    ...
    if (!skip) {
      axios
        .delete(
          "/api/dives/supprimer/diveteam/" + updatedPalanquees[indexElem].id
        )
        .then((response) => {
      ...
      for (let id in palanqueeMap) {
        if (palanqueeMap[id].length <= 1) {
          return alertify.error(
            "You can't have a palanquee with less than 2 divers"
          );
        }
      }
    }
