        ...
        bcrypt.hash(password, 10, (err, hash) => {
            ...
            db.query(
                ...
                (err, result) => {
                    ...
                    let token = jwt.sign({timestamp: Date.now() + 30000, prenom: first_name, email: email, rank: rank}, secretKey, {expiresIn: '24h'});
                ...
                bcrypt.compare(password, user.password, (err, result) => {
                    ...
                    db.query(query, [email], (err, results) => {
                        ...
                        switch (results[0].rank) {
                            case 2:
                                token = jwt.sign({timestamp: Date.now() + 30000, prenom: results[0].last_name, email: results[0].email, rank: 2}, secretKey, {expiresIn: '24h'});
                                break;
                            case 1:
                                token = jwt.sign({timestamp: Date.now() + 30000, prenom: results[0].last_name, email: results[0].email, rank: 1}, secretKey, {expiresIn: '24h'});
                                break;
                            case 0:
                                token = jwt.sign({timestamp: Date.now() + 30000, prenom: results[0].last_name, email: results[0].email, rank: 0}, secretKey, {expiresIn: '24h'});
                                break;
                            default:
